...
\begin{tabular}{p{0.45\textwidth} p{0.45\textwidth}}
  ...
  \) &
  $c^2 = \frac{k}{\rho C}$ -- теплопроводность, $f$ -- источник
\end{tabular} \bigskip\\
...
\begin{wrapfigure}{l}{0.3\textwidth}
  ...
  \includegraphics[width=0.9\linewidth]{pics/s1l1.png}
  \label{fig:example}
\end{wrapfigure}

Пусть $G$ --- область в $\mathbb{R}^n$, x=($x_1$, \ldots, $x_n$) --- точка в $\mathbb{R}^n$. УЧП2П называется соотношение, связывающее значение неизвестной функции n переменных u=u(x), и её производных 1\textsuperscript{го} и 2\textsuperscript{го} порядка:
\[
...
Решением \LDUSP{} называется функция класса $u(x) \in C^2(G): u(x)$, которая удовлетворяет данному уравнению.
\begin{note}
