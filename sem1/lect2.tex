...
Таким образом, для \LDUChPSPPK{} можно заменить МСК $A$ на симметричную, вида: $\displaystyle A_c=\frac{A+A^T}{2}$ \\
...
$$ 
  \tilde{L} \tilde{u} =
    ...
    \sum^{n}_{k=1} \tilde{b}_{k}\frac{\partial\tilde u}{\partial \xi_k} + c \tilde u = \tilde f(\xi)
$$

\subsection{Классификация ЛДУЧП2ППК}

\textbf{I. Эллиптический.}
Если для квадратичной формы $K=y^T A y$, $r=n$ и в её каноническом виде все слагаемые одинакового знака($p=r=n,\,q=0$ либо $p=0,\,q=r=n$), то \LDUChPSPPK{} называется уравнением эллиптического типа.\smallskip\\
\emph{Пример:} Уравнение Лапласса: $\displaystyle \Delta u = 0$\smallskip

\textbf{II. Гиперболический.}
Если для квадратичной формы $K = y^TAy$, $r=n$ и в её каноническом виде $1\le p \le n-1,\,q \in [1,n-1]$, то \LDUChPSPPK{} называется уравнением гиперболического типа.\medskip\\
\emph{Пример:} Волновое уравнение: $\displaystyle \frac{\partial^2 u}{\partial t^2} = c^2 \Delta u + f(x, y, z, t)$
\begin{note}
  Если $p=1,\,q=n-1$ или $p=n-1,q=1$ -- нормальный гиперболичкеский вид
\end{note}

\textbf{III. Параболический.}
Если для квадратичной формы $K = y^TAy$, $r<n$, то \LDUChPSPPK{} называется уравнением гиперболического типа.\medskip\\
\emph{Пример:} Уравнение теплопроводности: $\displaystyle \frac{\partial u}{\partial t} = c^2 \Delta u + f(x, y, z, t)$
\begin{note}
  Если $p=r,\,q=0$ или $p=0,q=r$ -- нормальный параболический вид
\end{note}
...
$$
Lu=\sum_{i,j=1}^{n} a_{ij}(x)\frac{\partial^2u}{\partial x_i \partial x_j} + \sum_{i=1}^{n} b_{i}(x)\frac{\partial u}{\partial x_i} + c(x)u = f(x), \quad x \in G,
$$
...
$$ 
  ...
  \Longrightarrow \frac{\partial^2 u}{\partial x_i \partial x_j}=
      \sum^n_{k, l=1} \frac{\partial^2 \tilde u}{\partial \xi_k\partial \xi_l}
...
$$ 
  ...
  \Longrightarrow \frac{\partial^2}{\partial x_i \partial x_j}=
      \sum^n_{k, l=1} \frac{\partial \xi_k}{\partial x_i}\cdot\frac{\partial \xi_l}{\partial x_j}
...
\sum^n_{k=1} \tilde b_k (\xi) \frac{\partial \tilde{u}}{\partial \xi_k} + \tilde{c}(\xi)\tilde{u} = \tilde{f}(\xi), \qquad k=\overline{1,n}
$$
где: $$\displaystyle \tilde b_k(\xi) = \sum^n_{i,j=1}a_{ij}(x)\frac{\partial^2 \xi_k}{\partial x_i\partial x_j}+\sum^n_{i=1}b_i(x)\frac{\partial\xi_k}{\partial x_i},\qquad k=\overline{1,n}$$\\
$$\tilde c(\xi)=c(x(\xi)),\qquad\tilde f(\xi) = f(x(\xi))$$

Считаем матрицу старших коэффициентов $A(x)=(a_{ij}(x))$ -- симметрической. Аналогично, $ \tilde A(\xi)=(\tilde a_{ij}(\xi)) $ в ДУЧП2ПЛОСП(ЛДУЧП2П).\\
Фиксируем некоторую точку $x=x^{(0)}\in G$.\\
Обозначим $\displaystyle\xi^{(0)} = \xi(x^{(0)}) \Longrightarrow \tilde a_{kl}(\xi^{(0)}) = \sum^n_{i,j=1}a_{ij}(x^{(0)})s_{ik}s_{jl}$,
где $\displaystyle s_{ik} = \frac{\partial \xi_k}{\partial x_i}(x^{(0)})$ (и т.п.), или
$$ 
\tilde A(\xi^{(0)}) = S^T A(x^{(0)}) S \Longrightarrow \sum^n_{k,l=1}\tilde a_{kl}(\xi)\frac{\partial^2 \tilde u}{\partial \xi_k \partial \xi_l}+
