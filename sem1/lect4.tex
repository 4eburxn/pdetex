...
\begin{equation}
  ...
  \Phi(\xi,\eta,\tilde u, \frac{\partial\tilde u}{\partial \xi},\frac{\partial\tilde u}{\partial \eta}) = 0
  \tag{$\widetilde 1$}
\end{equation}
где: 
...
\begin{definition}[Характеристика и характерестическая кривая]
  Пусть функция $\omega(x,y) \in C^1(G):$ что на кривой $\omega(x,y)=0$ выполняется $\operatorname{grad}(\omega(x,y))=0$ и\\
   \begin{equation}
    a\omega^2_x+2b\omega_x\omega_y+c\omega^2_y=0
    \tag{2}
   \end{equation}
  Тогда кривая $\omega(x,y)=0$ называется характерестической кривой(характеристикой уравнения $(1)$), а само уравнение $(2)$ -- характерестическим уравнением $(1)$.
\end{definition}
...
\end{lemma}
\begin{proof}
  Т.к. $\omega(x,y)\in C^1(G),\,\omega_y\ne 0$ и $\omega(x,y)$ удовлетворяет уравнению $(2)$, ир из него получаем:\\
...
\end{proof}
Вспомним, что соотношение $\omega(x,y)=C$ является общим интеграллом ОДУ, если это соотношение ($\omega(x,y)=C$) неявно задает функцию $y=y(x)$, которая
является решением указанного ОДУ.\\
Тогда по теореме ``О неявных функциях'', из $\omega_y\ne0\rightarrow$

\hspace*{1em}  2. Эллиптический тип: $\displaystyle \hat{u}_{\alpha\alpha}+\hat{b}_{\beta\beta}=\hat\Phi $;

\hspace*{1em}  3. Параболический тип: $\displaystyle \widetilde{b}_{\eta\eta}=\widetilde{\widetilde{\Phi}} $;
\begin{note}
  Если в некоторой области $G$ уравнение имеет гиперболический тип(ГТ), то в этой области $G$ существует 2 семейства $\mathbb R$ - характеристик, а если эллиптический тип, то в этой области $G$ существует 2 семейства $\mathbb C$ - сопряженных характеристик.
\end{note}
...
Перейдем в уравнении $(1)$ к НП: %FIXME КТО ТАКОЙ
$\xi=\phi(x,y),\,\eta=\psi(x,y)$.\\
Тогда уравнение $(1)$ в некоторой окрестности из $G$ примет вид:

\hspace*{2em} $2\widetilde b \widetilde u_{\xi\eta}+\widetilde \Phi=0$, откуда $\widetilde u_{\xi\eta}=\widetilde{\widetilde \Phi}$
