...
\begin{definition}[Струна]
	...
	Это означает, что натяжение в каждой точке направлено по касательной к мгновенному профилю струны.
\end{definition}
  %Дипсичное begin
\begin{enumerate}
	\item Струна абсолютно гибкая (не сопротивляется изгибу).
	\item Колебания малые: $|u| \ll l$, $u_x^2 \ll 1$.
	\item Струна нерастяжима (подчиняется закону Гука, но удлинение пренебрежимо мало).
\end{enumerate}
\begin{wrapfigure}{l}{0.3\textwidth}
	\centering
	% Сама картинка масштабируется до 90% от ширины окружения
	\includegraphics[width=1.\linewidth]{pics/s1l5_1.png}
\end{wrapfigure}
  %Дипсичное end
...
Тогда $\displaystyle \alpha \approx sin(\alpha) \approx tg(\alpha) \approx u_x << 1,\ cos(\alpha)=\frac{1}{\sqrt{1+u_x^2}} \approx 1$
...
\]
  ...
  %Дипсичное begin
Следовательно, в рамках сделанных предположений длина дуги не меняется, значит, натяжение не зависит от времени: $T(x, t) = T(x)$.
Рассмотрим проекцию сил на ось $Ox$ для участка $(x_1, x_2)$:
\[
T(x_2)\cos\alpha(x_2,t) - T(x_1)\cos\alpha(x_1,t) = 0.
\]
Так как $\cos\alpha \approx 1$, получаем $T(x_2) \approx T(x_1)$, то есть натяжение не зависит и от $x$. 
Таким образом, $T(x,t) \equiv T_0 = \text{const}$.
  %Дипсичное end
...
\begin{wrapfigure}{l}{0.3\textwidth}
	\centering
	% Сама картинка масштабируется до 90% от ширины окружения
	\includegraphics[width=1\linewidth]{pics/s1l5_2.png}
\end{wrapfigure}
...
\[
  %Дипсичное begin
u_x(x_0+0,t) - u_x(x_0-0,t) = -\frac{\Phi_0(t)}{T_0}.
\]
Это условие означает, что в точке приложения сосредоточенной силы происходит излом струны (скачок производной). 
При этом само уравнение (1.f) в классическом смысле в этой точке не выполняется, а заменяется данным условием. 
(Если понимать уравнение в обобщённом смысле, то правая часть содержит дельта-функцию, и интегрирование даёт тот же результат.)
  %Дипсичное end
...
\begin{wrapfigure}[8]{l}{0.3\textwidth}
	\centering
	\includegraphics[width=1\linewidth]{pics/s1l5_3.png}
\end{wrapfigure}
...
\begin{theorem}[Существования и единственности]
	Решение простейшей краевой задачи для уравнения колебаний струны длины l, закрепленной на концах, $(1.f),\ (2),\ (3.1.0),\ (4.1.0)$ --- существует и единственно в рассматриваемых классах функций.
\end{theorem}
